...
export const PostAudioPlayer = ({ contentSelector = "article", title }: PostAudioPlayerProps) => {
  ...
  // Synchronous mirror of isPlaying for use inside animation frames
  // React state updates are batched/async, so the fallback tracker
  // would read stale `isPlaying = false` from its closure on first play.
  // This ref is set BEFORE the tracker starts, so it always has the correct value.
  ...
  useEffect(() => {
    ...
    // Robust Initialization: wait for article content
    const initPlayer = () => {
    ...
    if (!initPlayer()) {
      const retryInterval = setInterval(() => {
        if (initPlayer()) clearInterval(retryInterval)
      }, 500)
  ...
  // Instrumentation: Resilient background trigger
  useEffect(() => {
    if (fullText) {
      const runInstrument = () => {
        instrumentContent()
        if (wordSpans.current.length === 0 && fullText) {
  ...
  const startFallbackTracking = (startIndex: number, currentRate: number) => {
    ...
    const step = (now: number) => {
      if (!startTime.current || !isPlayingRef.current) return
      ...
      if (currentIdx < fullText.length && isPlayingRef.current) {
  ...
  const speakFromIndex = (startIndex: number, currentRate: number) => {
    ...
    // Set ref BEFORE starting tracker so the first frame sees true
                    ...
                    <div className="flex items-center gap-3 w-full sm:w-auto">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-col">
                          <h4 className="text-[13px] sm:text-sm font-black text-foreground truncate max-w-[200px] tracking-tight">{title}</h4>
                          <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mt-0.5">Post Reader</span>
                        </div>
                      </div>

...
export function Footer() {
  ...
  React.useEffect(() => {
    if (typeof window !== "undefined") {
      const subscribed = localStorage.getItem("infixe_subscribed")
      if (subscribed) {
  ...
  const handleSubscribe = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatus("success")
      localStorage.setItem("infixe_subscribed", "true")
      setHasSubscribed(true)
    } catch (err: any) {
      setStatus("error")
      ...
      <div className="container mx-auto px-6">
        <div className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-5 gap-12 pb-16 border-b border-white/10">
          <div className="lg:col-span-2">
            <Link href="/" className="text-2xl font-extrabold flex items-center hover:opacity-80 transition-opacity">
              <span className="bg-gradient-to-r from-[#FF671F] via-[#FFFFFF] to-[#046A38] text-transparent bg-clip-text">INFIXE</span>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between py-8 text-xs text-[#64748B]">
          <p>© 2026 InFixe. All rights reserved.</p>
